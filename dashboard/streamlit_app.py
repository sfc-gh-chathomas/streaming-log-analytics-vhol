...
def render_dashboard():
    """All the live panels. Refreshed in place by a fragment (no full-page rerun)."""
    # ======================== Headline: freshness ========================
...
# Refresh the panels IN PLACE so charts/tables do not bounce. `st.fragment(run_every=...)`
# reruns only this function on a timer, leaving the rest of the page (title, sidebar) static
# and skipping the whole-page "Running" reflow a full st.rerun causes. Fall back to
# experimental_fragment, then to a sleep + rerun loop, on older SiS runtimes.
_fragment = getattr(st, "fragment", None) or getattr(st, "experimental_fragment", None)
if _fragment:
    _fragment(run_every=refresh_s)(render_dashboard)()
else:
    render_dashboard()
